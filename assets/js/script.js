...
function renderinputs(totalinput) {
    ...
    for(let i=0; i<totalinput; i++)
    {
        ...
        input.addEventListener("keyup", validateresponse)
        playform.appendChild(input);
    }
...
// Following is listerner function on keydown of each input text that will change input style and 
// attribute to readonly if guess matches
function validateresponse(event)
{    
    let key = event.key.toLowerCase();
    let index = this.id.replace("input", "");       
    
    if(computerchoicechars[index].toLowerCase()==key)
    {   
        document.getElementById(this.id).innerText = event.key;
        ...
        userguess[index] = computerchoicechars[index].toLowerCase();
        validatewordguess();        
          
    if(index<computerChoice.length-1 && event.key!='')
    {
        parseInt(index);
        index++;
        let inputfocus = "input"+(index);        
        document.getElementById(inputfocus).focus();
    }
    }
}
// check if whole world is guessed then end game
function validatewordguess()
{    
    let userguessword = userguess.join('');    
    if(userguessword==computerChoice.toLowerCase())
    {
         clearInterval(timerInterval); // stop timer

         let gamescorcard = JSON.parse(localStorage.getItem("gamescorcard"));
         gamescorcard.totalwin++;
         localStorage.setItem("gamescorcard", JSON.stringify(gamescorcard));
         // use of template literals
         gamestatus.innerText = `Congratulations! You've guessed it right.
          Total Win: ${gamescorcard.totalwin}
          Total Loss: ${gamescorcard.totalloss}`;
          playagain.setAttribute("style", "display: block");

    }
}
// end the game if no time left
function endthegame()
{
    clearinput();
    let gamescorcard = JSON.parse(localStorage.getItem("gamescorcard"));
    gamescorcard.totalloss++;
    localStorage.setItem("gamescorcard", JSON.stringify(gamescorcard));
    // use of template literals    
     gamestatus.innerHTML = "The word was '<b>"+computerChoice.toUpperCase()+"<b>'<br><br>GAME OVER! You Lose<br>"+
     "Total Win: " + gamescorcard.totalwin + 
     "Total Loss: " + gamescorcard.totalloss;
    playagain.setAttribute("style", "display: block");
}
// following will clear last game inputs
function clearinput()
{
...
// Following function will reset elements and start new game
function startnewgame(){
    // Get random word from array of options  
    let index = Math.floor(Math.random() * wordbank.length);
    computerChoice = wordbank[index];
    computerchoicechars = computerChoice.split("");

   // create empty array for userguess - each letter will replace if guess matches
     for(let i=0; i< wordbank.length; i++)
     userguess.push('');
  // render new word
  renderinputs(computerChoice.length);
}

// Add event listener to multiple elements: button playgame and playagain
[btnplaygame, btnplayagain].forEach((element) => {
...
function init() {    
    let isgamestats = JSON.parse(localStorage.getItem("gamescorcard"));
    if(isgamestats==null)
    {
        // object storing stats
        gamescorcard= {
            totalwin: 0,
            totalloss: 0
        }  
